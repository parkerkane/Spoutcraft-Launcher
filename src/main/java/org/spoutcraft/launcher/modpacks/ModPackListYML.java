...
public class ModPackListYML {
    ...
    private static final String MODINFO_HTML        = "modinfo.html";
    ...
    public static void downloadModPackResources(String name, String label, File path) {
        ...
        if (downloadFileList.size() > 0 &&
            DownloadUtils.downloadFiles(downloadFileList, 30, TimeUnit.SECONDS) != downloadFileList.size()) {
    ...
    public static void loadModpackLogos() {
        for (String modPack : modpackMap.keySet()) {
            ...
            modpackLogoList.put(modPack,
                new ImageIcon(Toolkit.getDefaultToolkit().getImage(modPackLogo.getAbsolutePath())));
        }
    }
